createFromSSN <- function (
  ...
  if(class(ssn)[1] == "SpatialStreamNetwork"){
    ...
    nid_tables <- dir(ssn.path, ".dat", full.names = TRUE)
    n_networks <- length(nid_tables) 
  }
  ...
  #     return(sqrt(sum((locations_this_network[indicies[1], 
  #                                             ] - locations_this_network[indicies[2], ])^2)))
  #   }
  ...
  #                                                                             first.location))), ID = as.character(edge.index + 
  #                                                                                                                    cumulative_nedges))
  #   }
  ...
  #   while (TRUE) {
  #     next_points <- (remaining_edges[, 2] %in% remaining_points) & 
  #       (remaining_edges[, 1] %in% known_point_indicies)
  #     upstream_points <- remaining_edges[next_points, 2]
  #     downstream_points <- remaining_edges[next_points, 
  #                                          1]
  #     additional_binary_bits <- c()
  #     counter <- 1
  #     while (counter <= length(downstream_points)) {
  #       if (counter == length(downstream_points) || downstream_points[counter] != 
  #           downstream_points[counter + 1]) {
  #         additional_binary_bits <- c(additional_binary_bits, 
  #                                     "1")
  #         counter <- counter + 1
  #       }
  #       else {
  ...
  #     known_binaryids <- c(known_binaryids, paste(previous_binary_ids, 
  #                                                 additional_binary_bits, sep = ""))
  #     known_point_indicies <- c(known_point_indicies, upstream_points)
  #     known_rids <- c(known_rids, remaining_rids[next_points])
  #     remaining_edges <- remaining_edges[!next_points, 
  #                                        , drop = FALSE]
  #     remaining_rids <- remaining_rids[!next_points]
  #     remaining_points <- remaining_points[remaining_points %in% 
  #                                            remaining_edges[, 2]]
  ...
  #   binary_ids_tables[[netid]] <- data.frame(rid = rids[[netid]], 
  #                                            binaryID = binary_ids)
  #   if (length(unique(binary_ids_tables[[netid]]$binaryID)) != 
  #       length(binary_ids_tables[[netid]]$binaryID)) 
  #     stop("Internal error")
  #   write.table(binary_ids_tables[[netid]], file = file.path(path, 
  #                                                            paste("netID", netid, ".dat", sep = "")), col.names = T, 
  #               sep = ",", row.names = FALSE)
  # }
  # setwd(path)
  ...
  for (netid in 1:n_networks) {
    ...
    partial_match_function <- function(binary_id1, binary_id2) {
      ...
      for (j in 1:min_len) {
        if (substr(binary_id1, j, j) != substr(binary_id2, 
                                               j, j)) 
          return(j - 1)
      }
      return(min_len)
    ...
    for (i in 1:nrow(binary_id_table)) {
      ...
      distance_matrix[as.character(current_rid), ] <- pmax(current_updist - 
                                                             downstream_updists, rep(0, length(downstream_updists)))
    }
  ...
  for (netid in 1:n_networks) {
    ...
    if (n_locations_this_network > 0) {
      ...
      if (length(pred_location_data_this_network) > 0) {
        colnames(pred_location_data_this_network) <- c("NEAR_X", 
                                                       "NEAR_Y", "upDist")
      }
      else {
      ...
      if (length(obs_location_data_this_network) > 0) {
        colnames(obs_location_data_this_network) <- c("NEAR_X", 
                                                      "NEAR_Y", "upDist")
      }
      else {
      ...
      obs_location_data_this_network <- pred_location_data_this_network <- data.frame(NEAR_X = numeric(0), 
                                                                                      NEAR_Y = numeric(0), upDist = numeric(0))
      obs_pids <- integer(0)
      pred_pids <- integer(0)
    }
    ...
    obs_data_this_network <- data.frame(locID = obs_location_data[, 
                                                                  "locID"], upDist = obs_location_data_this_network[, 
                                                                                                                    "upDist"], pid = obs_pids, netID = rep(netid, length(obs_pids)), 
                                        rid = obs_location_data[, "rid"], ratio = obs_location_data[, 
                                                                                                    "ratio"], shreve = line_data[match(obs_location_data[, 
                                                                                                                                                         "rid"], line_data[, "rid"]), "shreve"], addfunccol = line_data[match(obs_location_data[, 
                                                                                                                                                                                                                                                "rid"], line_data[, "rid"]), "addfunccol"], stringsAsFactors = FALSE)
    if (ncol(obs_sites_this_network) > 3) {
      obs_data_this_network <- cbind(obs_data_this_network, 
                                     obs_sites_this_network[, -match(c("edge", "ratio", 
                                                                       "locID"), colnames(obs_sites_this_network)), 
                                                            drop = FALSE])
    }
    ...
    pred_data_this_network <- data.frame(locID = pred_location_data[, 
                                                                    "locID"], upDist = pred_location_data_this_network[, 
                                                                                                                       "upDist"], pid = pred_pids, netID = rep(netid, length(pred_pids)), 
                                         rid = pred_location_data[, "rid"], ratio = pred_location_data[, 
                                                                                                       "ratio"], shreve = line_data[match(pred_location_data[, 
                                                                                                                                                             "rid"], line_data[, "rid"]), "shreve"], addfunccol = line_data[match(pred_location_data[, 
                                                                                                                                                                                                                                                     "rid"], line_data[, "rid"]), "addfunccol"], stringsAsFactors = FALSE)
    if (ncol(pred_sites_this_network) > 3) {
      pred_data_this_network <- cbind(pred_data_this_network, 
                                      pred_sites_this_network[, -match(c("edge", "ratio", 
                                                                         "locID"), colnames(pred_sites_this_network)), 
                                                              drop = FALSE])
    }
  ...
  sites <- SpatialPointsDataFrame(combined_site_location_data[, 
                                                              c("NEAR_X", "NEAR_Y"), drop = FALSE], sites_data, match.ID = TRUE)
  writeOGR(sites, ".", "sites", verbose = FALSE, driver = "ESRI Shapefile")
  if (length(combined_pred_location_data) > 0) {
    preds <- SpatialPointsDataFrame(combined_pred_location_data[, 
                                    ...
                                    match.ID = TRUE)
    writeOGR(preds, ".", "preds", verbose = FALSE, driver = "ESRI Shapefile")
  }
  setwd(old_wd)
  if (importToR) {
