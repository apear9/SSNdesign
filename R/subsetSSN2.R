subsetSSN2 <- function(ssn, filename, subset, clip.edges = FALSE, clip.preds = FALSE) {
  ...
  if(clip.edges){
    ...
    writeOGR(edges.sub, "edges.shp", "edges", driver = "ESRI Shapefile", overwrite_layer = TRUE)
    SSN:::write.dbf.SSN(edges.sub@data, "edges", max_nchar = 30)
  ...
  if(clip.preds){
    ...
    if (pred.len > 0) {
      for (i in 1:pred.len) {
        ...
        writeOGR(preds.sub, paste0(pred.name, ".shp"), pred.name, driver = "ESRI Shapefile", overwrite_layer = TRUE)
        SSN:::write.dbf.SSN(preds.sub@data, pred.name, max_nchar = 30)
  ...
  writeOGR(sites.sub, "sites.shp", "sites", driver = "ESRI Shapefile", overwrite_layer = TRUE)
  SSN:::write.dbf.SSN(sites.sub@data, "sites", max_nchar = 30)
